...
def interpret_lstm(args, data_iter, text_field, label_field, DEVICE, EPSLN=1.00, MAX_RM=4):
    ...
    for text_faith, label in tqdm(zip(list_texts, list_labels)):
        ...
        x_delta, importance_score, x_after, changes_pred = iterative_gradient(model, x.detach(), pred_label,
                                                                              step_size=0.02, epsilon=EPSLN)    #0.02
        ...
        avg_changes_rm_pred_inteGrad += evaluate_word_removal(model, x.detach(), pred_label, avg_grad,
...
def find_counterfactual_distance(model, x_org, x_after, pred_label, DEVICE, step_size):
    ...
    cnt = 1
    model.hidden = model.init_hidden()
    while torch.argmax(model(x_after)[0]) == pred_label[0]:
    ...
    while torch.sqrt(torch.sum((x1-x2) ** 2)) > step_size:
        x_middle = (x1 + x2) / 2
        model.hidden = model.init_hidden()
        if torch.argmax(model(x_middle)[0]) != pred_label[0]:
            x2 = x_middle
        else:
            x1 = x_middle
    x_middle = (x1 + x2) / 2
